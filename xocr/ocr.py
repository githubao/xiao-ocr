...
"""
@description: 自己实现一个ocr识别
from 500-lines

@author: baoqiang
@time: 2019-06-16 16:38
"""
...
class OCRNeuralNetwork:
    ...
    def __init__(self, num_hidden_nodes, data_matrix, data_labels, training_indices, use_file=True):
        ...
        if not os.path.isfile(OCRNeuralNetwork.NN_FILE_PATH) or not use_file:
            ...
            self.train([TrainData(self.data_matrix[i], int(self.data_labels[i]))
    ...
    def _draw(self, sample):
        ...
        plt.show(zip(*pixelArray), cmap=cm.Greys_r, interpolation="nearest")
        plt.show()
    ...
    def _load(self):
        ...
        self.hidden_layer_bias = [np.array(nn['b1'][0])]


class TrainData:
    def __init__(self, y0, labels):
        self.y0 = y0
        self.labels = labels
